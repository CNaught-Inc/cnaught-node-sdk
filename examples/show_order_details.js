    ...
    try {
        ...
        if (order.project_allocations.length > 0) {
            ...
            order.project_allocations.forEach(pa => {
                ...
                console.log(`|--- Registry: ${pa.project.registry_name} - ${pa.project.registry_id} - ${pa.project.registry_url}`);
                console.log(`|--- Location: ${pa.project.location_name} - (${pa.project.location_latitude}, ${pa.project.location_longitude})`);
                ...
                console.log(`|--- UN SDG Goals: ${pa.project.un_sdg_goals.join(',')}`);
                if (pa.retirements.length > 0) {
                    console.log('|--- retirements');
                    pa.retirements.forEach(r => {
                        console.log(`|----- ${r.serial_number_range} - ${r.url}`);
                    });
                }
                console.log('---------------------------------------');
            });
